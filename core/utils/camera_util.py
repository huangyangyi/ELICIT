...
def get_rays_from_KRT(H, W, K, R, T, stride=1):
    ...
    i, j = np.meshgrid(np.arange((stride-1)/2, W, stride, dtype=np.float32),
                       np.arange((stride-1)/2, H, stride, dtype=np.float32),
                       indexing='xy')
...
def get_projection_depth(points, K, R, T):
    r"""Get projection depth of a set of points based on camera metrices (K, R and T)
    ...
    Returns:
        ...
        - xy: Array (N, 2)
    """
    points_camera = np.dot(points, np.linalg.inv(R)) + T.ravel()
    xyd = np.dot(points_camera, K.T)
    depth = xyd[:, -1]
    xy = np.floor(xyd[:, :2] / xyd[:, -1:]).astype(np.int32)
    return depth, xy
