...
def main(argv):
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        ...
        cameras[out_name] = {
                ...
                'distortions': D,
                'all_views': {
                    i: dict(intrinsics=Ks[i], extrinsics=Es[i], distortions=Ds[i]) for i in range(len(Ks))
                }
        }
        ...
        bigpose_vertices, bigpose_joints = smpl_model(big_poses, betas)
        mesh_infos[out_name] = {
            'Rh': Rh,
            'Th': Th,
            'poses': poses,
            ...
            'bigpose_joints': bigpose_joints,
            'bigpose_vertices': bigpose_vertices
        }
