...
def main(argv):
    ...
    output_name = cfg['output']['name'] + '_smpl'
    select_view = cfg['training_view']
    n_frames = cfg['max_frames']

    print('Processing {}'.format(subject))
    single_frame_id = '0'
    input_dir = '{}/{}'.format(source_dir, subject)
    output_path = '{}/{}/'.format(output_dir, output_name)
    custom_betas = np.load('{}/new_params/{}.npy'.format(input_dir, single_frame_id), allow_pickle=True).item()['shapes']
    ...
    # load cameras
    cams = annots['cams']
    cam_K = np.array(cams['K']).astype('float32')[select_view]
    cam_R = np.array(cams['R']).astype('float32')[select_view]
    cam_T = np.array(cams['T']).astype('float32')[select_view] / 1000.
    cam_D = np.array(cams['D']).astype('float32')[select_view]
    smpl_params = np.load('{}/new_params/{}.npy'.format(input_dir, 0), allow_pickle=True).item()
    Th = smpl_params['Th'][0]  #(3,)

    n_cams = 20
    Ks = np.stack([cam_K for _ in range(n_cams)])
    Ds = np.stack([cam_D[:, 0] for _ in range(n_cams)])
    Es = np.stack([np.eye(4) for _ in range(n_cams)])  #(N, 4, 4)
    cam_T = cam_T[:3, 0]
    Es[:, :3, :3] = cam_R
    Es[:, :3, 3]= cam_T
    for i in range(1, n_cams):
        rot_angle = 2 * np.pi / n_cams * i 
        Es[i] = _update_extrinsics(Es[0], rot_angle, trans=Th, rotate_axis='z')

    batch_size = 4
    cameras = {}
    mesh_infos = {}
    all_betas = []
    for fs in tqdm(range(0, n_frames, batch_size*sample_interval)):
        for i in range(n_cams):
            ft = min(fs + batch_size*sample_interval, n_frames)
            K = Ks[i]
            E = Es[i]
            batch_out_names = []
            batch_Rh = []
